param (
    $resourceBaseName="podcast$( Get-Random -Maximum 1000)",
    $location='eastus'
)
...
dotnet build ..\src\Podcast.Api\Podcast.Api.csproj

Write-Host 'Packaging API project for deployment' -ForegroundColor Green
dotnet publish ..\src\Podcast.Api\Podcast.Api.csproj --self-contained -r win-x86 -o ..\publish
